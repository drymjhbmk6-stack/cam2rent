...
- **🐛 BEKANNTER BUG — Doppelte Buchungsmails (Stand 2026-05-27, NOCH NICHT GEFIXT):** Ein Kunde meldet 3× Admin-Mail + 2× Kundenmail für **dieselbe** Buchung, obwohl nichts manuell gesendet wurde. Ursache ist ein Race zwischen drei Mail-Sendern, die alle fast zeitgleich für dieselbe Buchung laufen können:
  1. **Stripe-Webhook `payment_intent.succeeded` → `handleCartBooking`** (`app/api/stripe-webhook/route.ts:602-862`) legt die Buchung als erster in die DB an und schickt 1× Admin-Mail (Kunden-Mail bewusst nicht — Webhook hat keinen Mietvertrag).
  2. **`POST /api/confirm-cart` (Call A)** vom Browser nach Stripe-Redirect: der Existing-Check bei `route.ts:157` sieht die Webhook-Zeile noch nicht (Race), `INSERT` knallt mit `23505` auf `payment_intent_id` → Vuln-17-Recovery-Branch (`route.ts:824-839`, eingebaut Sweep 6, Stand 2026-05-03) springt mit `break` aus dem Group-Loop, **fällt aber trotzdem in den `after()`-Mail-Block bei `route.ts:1175-1289`** → schickt 1× Kunde + 1× Admin.
  3. **`POST /api/confirm-cart` (Call B)** — vom selben Browser kurz danach, ausgelöst durch die `useEffect`-Dep-Kette in `app/buchung-bestaetigt/page.tsx:212-243`: `handleConfirm` ist `useCallback([paymentIntentId, items, clearCart, user])`. `clearCart()` aus Call A ändert `items` (Cart-Context propagiert), und gleichzeitig flippt `authLoading`/`user` — neue Callback-Identität → Effekt re-feuert, bevor `setConfirmed(true)` aus Call A propagiert ist. Selbes 23505 → selber Vuln-17-Branch → noch 1× Kunde + 1× Admin.
  - **Zwei zusammenhängende Fehler:** (a) Der Vuln-17-Branch in `confirm-cart` (Sweep 6, 2026-05-03 — sollte verhindern, dass die Buchung ohne Mietvertrag in der DB liegt, wenn der Webhook schneller ist) unterscheidet nicht zwischen „Webhook hat eingefügt" und „früherer `confirm-cart`-Call hat eingefügt"; in beiden Fällen läuft der Mail-Block. (b) Der `useEffect` in `/buchung-bestaetigt/page.tsx` hat keinen In-Flight-Guard — `clearCart()` läuft INNERHALB von `handleConfirm`, `setConfirmed(true)` erst DANACH → kurzes Race-Fenster für einen zweiten Call.
  - **Fix-Vorschlag (noch nicht umgesetzt):** Server: im Vuln-17-Branch ein `alreadyExistedBeforeInsert`-Flag setzen und die Mails (`after()`-Block) NUR dann verschicken, wenn die Buchung wirklich von diesem Request frisch angelegt wurde — die ursprüngliche Idee des Patches (Vertrag nachholen) bleibt damit erhalten. Client: entweder `handleConfirm` aus den `useEffect`-Deps nehmen oder einen `inFlightRef` (`useRef<boolean>`) als Re-Entry-Guard ergänzen.
  - **Wer in dieser Konstellation noch +1 Admin-Mail liefert:** wenn `verificationDeferred=OFF` lief, schickt zusätzlich `create-pending-booking` (`app/api/create-pending-booking/route.ts:410`) 1× Admin-Mail beim Anlegen der `pending_verification`-Buchung — das erklärt die +1 Differenz zwischen Admin- und Kundenmails in der Praxis.
