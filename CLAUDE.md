...
- **Crontab (Auto-Storno unbezahlter Buchungen):** `*/30 * * * * curl -s -X POST -H "x-cron-secret: $CRON_SECRET" https://cam2rent.de/api/cron/awaiting-payment-cancel` — prüft alle 30 Min `awaiting_payment`-Buchungen und storniert automatisch 48h (Versand) bzw. 24h (Abholung) vor Mietbeginn wenn noch unbezahlt. Deaktiviert den Stripe Payment Link, setzt Status `cancelled`, schickt Storno-Mail. Grace-Period: 1h nach Buchungs-Erstellung.
