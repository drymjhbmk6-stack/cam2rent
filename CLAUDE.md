...
- **Gutschein-Code schon im Warenkorb eingebbar (Stand 2026-08-08):** Der Kunde kann den Gutschein-Code jetzt bereits auf `/warenkorb` eingeben (vorher nur im `/checkout`). Prüfung serverseitig über das bestehende `POST /api/validate-coupon` (`cartTotal` = Warenwert nach Produktrabatt, `userEmail` aus `useAuth()` für user-gebundene/once-per-customer-Codes). Der eingelöste Coupon wird in **`sessionStorage['cam2rent_coupon']`** abgelegt; **Warenkorb UND Checkout teilen sich diesen Key** — der Checkout liest ihn beim Mount aus (init `appliedCoupon`) und schreibt bei Einlösen/Entfernen zurück, sodass der Code in beide Richtungen synchron bleibt und der Kunde ihn nicht erneut eintippen muss. Der Warenkorb zeigt eine **Rabatt-Vorschau** (`calcDiscount` auf `cartTotal − Produktrabatt`); der finale Rabatt inkl. aller Auto-Rabatte (Mengen-/Frühbucher-/Treue-/Sonderkondition) wird weiterhin im Checkout berechnet (Hinweis „Rabatt wird im Checkout berechnet", falls die Vorschau-Basis 0 ist). `clearCart()` (`CartProvider`) räumt den `cam2rent_coupon`-Key mit weg → nach abgeschlossener Bestellung landet kein Alt-Coupon in einem neuen Warenkorb. Reine Client-/UX-Ergänzung — kein API-/Schema-Change, keine Server-Recompute-Änderung (Coupon-Floor-Validierung in `confirm-cart` unverändert).
