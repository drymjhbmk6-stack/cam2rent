...
- ⚠️ **Stale Rechtstext-Fallbacks:** `getParagraphen()` (contract-template.tsx) + AGB-Fallback (`app/agb/page.tsx`) sind noch die ALTE Nummerierung (19-§ Vertrag / 20-§ AGB). Autoritativ ist die DB-Fassung (V9, §§1-24 Vertrag / §§1-25 AGB). Beim Sweep wurde nur die Terminologie im Fallback angepasst, NICHT die Nummerierung.
- **Fallback-Generator (Stand 2026-07-27):** `scripts/sync-legal-fallbacks.ts` (`npm run sync:legal`) liest die autoritativen Texte aus Supabase (`admin_settings.contract_paragraphs` + `legal_documents`/`legal_document_versions`) und schreibt sie nach `lib/legal/generated-fallbacks.ts`. `getParagraphen()` bevorzugt `CONTRACT_PARAGRAPHS_FALLBACK`, der AGB-Fallback bevorzugt `LEGAL_FALLBACKS['agb']` (als Markdown via `LegalPageContent`) — solange der committete **Stub leer** ist, greift 1:1 das hand-gepflegte JSX (keine Regression). Der Generator hat ein **Vollständigkeits-Gate** (Vertrag ≥ 24 §§, AGB ≥ 25 §§) und bricht sonst ab (nichts geschrieben). **Muss einmal in einer Umgebung mit `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` gelaufen werden** — im Build-/Sandbox-Env ohne DB-Zugang bleibt der Stub leer.
...
- **Mietvertrag-Pflicht im Warenkorb-Checkout + Warnung vor Übergabe/Versand (Stand 2026-06-13):** Lücke geschlossen — der Warenkorb-Checkout (`/checkout`) hatte **gar keinen** Signatur-Schritt; eine Buchung konnte ohne unterschriebenen Mietvertrag bezahlt werden (der Direkt-Buchungsflow Step 5 verlangt die Unterschrift seit jeher, der Cart-Pfad übernahm nur eine evtl. in `sessionStorage` liegende Signatur). Zwei Teile:
...
- **UI:** `components/checkout/ExpressSignup.tsx` ist ein 3-Schritt-Flow (`auth → upload → done`). Nach Konto-Anlage muss der Neukunde **im selben UI direkt den Ausweis hochladen** (Vorder-/Rückseite, nutzt bestehende `/api/upload-id`). „Später hochladen"-Skip als Fallback bleibt erhalten — löst aber Reminder-/Auto-Storno-Flow aus. Bestandskunden-Login (Mode „Anmelden") überspringt den Upload-Step.
  - **Kamera ODER Galerie pro Ausweis-Seite (Stand 2026-06-13):** Jede Seite (Vorder-/Rückseite) hat ein klickbares Vorschaufeld + zwei explizite Buttons „📷 Foto aufnehmen" (Kamera) und „🖼 Galerie". Technisch zwei versteckte `<input type="file">` pro Seite: Kamera = `accept="image/*" capture="environment"` (öffnet direkt die Rückkamera auf Mobile), Galerie/Datei = `accept="image/jpeg,png,webp,heic,heif"` ohne `capture` (Foto-Mediathek/Datei). Auf Desktop wird `capture` ignoriert → beide öffnen den Datei-Dialog. Gemeinsamer Render-Helper `renderIdSide(side)`.
  - **Pflicht-Upload bei Registrierung UND Checkout-Schnellregistrierung (Stand 2026-06-14):** Neuer Prop `requireUpload?: boolean`. Wenn `true`, wird der „Später hochladen"-Skip ausgeblendet → der Ausweis-Upload ist Pflicht. Sowohl die Registrierungs-Seite (`/registrierung`) ALS AUCH die Checkout-Schnellregistrierung (`app/checkout/page.tsx`, `showExpressSignup`-Maske) setzen `requireUpload`. Bestandskunden-Login umgeht den Upload-Step generell (kein erneuter Ausweis nötig). **Hinweis:** Der separate Admin-Flag `verificationDeferred` (unverifizierter, BEREITS eingeloggter Bestandskunde darf zahlen + Ausweis nach der Buchung nachreichen) ist davon unabhängig — er steuert nur den Zahl-Schritt eines schon existierenden Kontos, nicht die Konto-Anlage. Default OFF.
  - **Ausweis VOR der Konto-Anlage bei `requireUpload` (Stand 2026-06-14):** Reihenfolge umgedreht, damit der Upload nicht mehr übersprungen werden kann. Statt „Konto erstellen → Upload-Step" (der durch einen Auto-Redirect übersprungen wurde) ist der Flow jetzt: Auth-Formular → Button **„Weiter: Ausweis hochladen"** (legt das Konto NOCH NICHT an, nur Formular-Validierung via `validateSignupForm`, dann `step='collect'`) → **`collect`-Step** mit Vorder-/Rückseite-Auswahl → Button **„Konto erstellen"** ist erst klickbar, wenn **beide** Seiten gewählt sind. Klick (`handleCreateAndUpload`) legt das Konto an (`createAccountAndLogin`) UND lädt im selben Handler den Ausweis hoch (`handleUpload`) → kein Zeitfenster mehr, in dem das Konto existiert aber der Upload offen ist. `accountCreated`-State verhindert doppelte Konto-Anlage bei Retry nach fehlgeschlagenem Upload (dann nur Re-Upload). Notausgang-Link im `collect`-Step nur nach `uploadFailed` (Konto schon da → eingeloggt, keine Sackgasse). Der alte `upload`-Step bleibt für den Checkout (`requireUpload=false`) unverändert (`handleSignup` → `step='upload'` mit Skip). Zusätzlich: Registrierungs-Seite unterdrückt ihren „schon eingeloggt → weiterleiten"-Effekt ab Flow-Start (`onAuthCompleted` → `authStarted`).
  - **Upload-Robustheit (Stand 2026-06-14):** Nach einem Support-Fall („Seite hängt beim Ausweis-Upload") gehärtet. (a) **Client-Verkleinerung** vor dem Upload via `shrinkImageFileIfNeeded(file, 2_500_000)` (`lib/shrink-image-client.ts`) — große Handy-Fotos (HEIC/3–5 MB) werden im Browser auf ~Hunderte KB geschrumpft, Upload ist auf Mobilfunk schnell. `handleFile` lehnt erst > 30 MB hart ab (vorher 5 MB → große Fotos wurden gar nicht erst akzeptiert). (b) **Timeout** via `AbortController` (90 s) → der „Wird hochgeladen…"-Spinner dreht nicht mehr endlos; `AbortError` zeigt eine klare „Internetverbindung prüfen (WLAN)"-Meldung. (c) **Notausgang bei Pflicht-Upload:** schlägt der Upload fehl/timeoutet (`uploadFailed`), erscheint trotz `requireUpload` ein „Ausweis später im Konto hochladen"-Link (`handleSkipUpload`) — der Kunde ist bereits eingeloggt und sitzt nicht in der Sackgasse fest. Unter normalen Bedingungen bleibt der Upload Pflicht (Link erscheint nur nach einem Fehlversuch). Das ersetzt das bisherige „Konto erforderlich"-Screen in `/checkout`, wenn Flag an. Für unverifizierte Kunden mit `verificationDeferred=true` erscheint statt „Buchung anfragen" der normale Zahlungs-Button mit amber-Hinweisbox zum Ausweis-Upload.
...
Wenn eine Buchung vor Ablauf der 14-tägigen Widerrufsfrist beginnt, muss der Kunde ausdrücklich zustimmen, dass cam2rent vor Fristende mit der Leistung beginnt und dass sein Widerrufsrecht dadurch erlischt.
- **Gemeinsame Checkbox** `components/booking/EarlyServiceConsentCheckbox.tsx` (Wortlaut deckt sich mit AGB § 16 Abs. 3 / Vertrag § 16 Abs. 3 / Widerrufsbelehrung). NICHT vorausgewählt, Absende-Button gesperrt bis angehakt.
...
- **Haftungs-Terminologie-Guard (Stand 2026-07-27):** Derselbe Test (`lib/__tests__/widerruf-consistency.test.ts`) prüft jetzt zusätzlich die verbotenen Haftungs-Anzeige-Begriffe im Produktivcode (`app lib components data`, ohne `__tests__`). Clean-`[]`-Guards für `Reparaturdepot`, `Schadenspauschale`, `Selbstbeteiligung`, `Standard-Haftung(sschutz/soption)` (alle 0 Vorkommen). Für die als Fremdbegriff/Identifier lebenden Wörter `Versicherung` (Buchhaltungs-Kategorie „Versicherungen", KI-Leitplanken-Prompts, Pflicht-Negation „keine Versicherung im Sinne des VVG") und `Eigenbeteiligung` (DB-Feld `standardEigenbeteiligung`/Helper `getEigenbeteiligung`) je eine **Datei-Allowlist mit Begründung** (`VERSICHERUNG_ALLOWLIST`/`EIGENBETEILIGUNG_ALLOWLIST`) — eine NEUE Datei mit dem Wort bricht den Guard. Der Guard prüft nur Anzeige-/Quelltext, NICHT DB-Feld-WERTE (`haftung='standard'` bleibt). Mitgefixt: eine echte Kunden-Anzeige in `app/kameras/[slug]/buchen/page.tsx` zeigte „Eigenbeteiligung: {opt.liability}" (dead `.replace()` + verbotenes Label) → jetzt nur noch `{opt.liability}` (die Liability-Strings tragen bereits „Höchstbetrag der Ersatzpflicht"). Offen/gemeldet: die **Admin-Konfig-UI** `components/admin/HaftungContent.tsx` nutzt „Eigenbeteiligung" noch als Feld-Label (allowlistet, internes Admin-Tool — bewusst nicht eigenmächtig umbenannt).
- **Seed-/Migrations-Bereinigung (Stand 2026-07-27):** Die historischen V1-Seeds in `erledigte supabase/` (`legal-seed-{agb,impressum,haftungsausschluss,widerruf}.sql`) tragen jeweils einen **⚠️-Warnheader „NICHT ERNEUT AUSFÜHREN"** (die DB-Fassung V7/V9 ist maßgeblich; ein erneutes `publish_legal_version()` würde sie überschreiben) + wurden um die verbotene Terminologie / veraltete Gesetzesnamen bereinigt: widerruf ohne § 312g (ersetzt durch § 356-Abs.-4-Hinweis) + Telefon; impressum § 5/§§ 7–10 DDG statt TMG, § 18 Abs. 2 MStV statt § 55 RStV, ODR-Plattform raus; agb Haftungsschutz/Höchstbetrag der Ersatzpflicht statt Schadenspauschale/Selbstbeteiligung, § 19 ODR raus; haftungsausschluss Basis-Haftungsschutz + Höchstbetrag der Ersatzpflicht + Reparaturdepot gestrichen + „bestimmungsgemäßer" Nutzung. Verbotene Begriffe verbleiben nur in den erklärenden SQL-Kommentaren/`change_note`, NICHT im Rechtstext-Body. **Bewusst NICHT** nachgebildet (nicht aus der Sandbox verifizierbar): V9-Neunummerierung, „Hinweis zur Verlegung", § 12-Wechselsatz, § 22 Supabase/Cloudflare — die DB bleibt autoritativ. Aktive Migrationen `supabase/supabase-{site-visits,blog-views}.sql`: Kommentar TTDSG → TDDDG.
- **Einwilligungs-Banner / Reichweitenmessung opt-in (§ 25 Abs. 1 TDDDG, Stand 2026-07-27):** Datenschutz V6 Ziffer 13 verspricht „Ohne Einwilligung findet keine Reichweitenmessung statt." → durchgesetzt. Zentrale Client-Lib **`lib/consent.ts`**: `getConsentDecision()` (`'all'`|`'necessary'`|`null`), `trackingAllowed()`, `setConsentDecision`/`revokeConsent`/`resetConsent`, Keys `cam2rent_consent` (+ `_at` Zeitstempel, `_v` Version `2026-07`), Custom-Event `cam2rent-consent-changed`. Bei `necessary`/Widerruf werden `cam2rent_vid`/`cam2rent_sid`/`cam2rent_visit_counted` sofort gelöscht. **Banner** (`components/CookieBanner.tsx`) zeigt bei Decision `null` zwei **gleichwertige** Buttons „Ablehnen"/„Zustimmen" (identische Klassen, keine Vorauswahl, kein Dark Pattern); Alt-Einwilligungen ohne aktuelle Version werden neu abgefragt. **`PageTracker`** (ID-basiert, `page_views`) gatet auf `trackingAllowed()` und startet nach nachträglicher Zustimmung OHNE Reload (Reaktion auf das Event). ⚠️ **`VisitTracker` — Update 2026-07-28 (Owner-Entscheidung):** der cookielose `site_visits`-Zähler ist **wieder EINWILLIGUNGSFREI** (zählt jeden Besuch), damit die Admin-Statistik „mit/ohne Cookies" echte Zahlen zeigt (sonst war „ohne Cookies" strukturell 0). Zulässig, weil der Zähler **NICHTS im Endgerät speichert** (kein Cookie/localStorage/sessionStorage — die frühere `cam2rent_visit_counted`-Dedup entfernt → § 25 Abs. 1 TDDDG nicht berührt) und **keinen Personenbezug** hat (`/api/visit` → nur Tages-/Stundenzählung, keine IP/ID). Zählt 1×/Seitenaufruf (Mount). ⚠️ **Offen/gemeldet:** die Cookie-Richtlinie (`/cookie-richtlinie` Abschnitt 4) + die Banner-/DS-Aussage „Ohne Einwilligung findet keine Reichweitenmessung statt" beschreiben noch den alten (gegateten) Zustand inkl. `cam2rent_visit_counted` — auf ausdrücklichen Wunsch NICHT angepasst; sollte für den anonymen Zähler klargestellt werden. **Datenschutz-Einstellungen**: `components/ConsentSettings.tsx` (Anker `#datenschutz-einstellungen` auf `/cookie-richtlinie`, Footer-Link „Datenschutz-Einstellungen") — erlauben/ablehnen so einfach wie erteilen (Art. 7 Abs. 3 DSGVO), Widerruf stoppt sofort + löscht IDs. `components/ResetConsentButton.tsx` entfernt (in `ConsentSettings` aufgegangen). Datenschutz-Code-Fallback §8 von „Art. 6 f / widersprechen" auf Opt-in umgestellt. Tests: `lib/__tests__/consent.test.ts` (Gate-Logik, node-env-Stub).
- **Rechtstext-Versions-Snapshot + eingefrorener Höchstbetrag am Vertrag (Stand 2026-07-27):** AGB § 1 Abs. 5 / Vertrag § 1 Abs. 4 (die bei Vertragsschluss geltende Fassung bleibt maßgeblich, Ausweis mit Versionsnummer) + § 8 Abs. 2 b) (maßgeblich ist der im Vertrag ausgewiesene Höchstbetrag). Migration `supabase/supabase-bookings-terms-version.sql` (idempotent, additiv): `bookings.terms_version`/`liability_terms_version`/`withdrawal_version`/`privacy_version` (TEXT) + `terms_snapshot_at` (TIMESTAMPTZ) + `liability_max_amount` (NUMERIC). **Einfrieren zentral in `generateContractPDF`** (`lib/contracts/generate-contract.ts` → `resolveContractLegalSnapshot`): liest die aktuellen Versionsnummern (`legal_documents`/`legal_document_versions`, Slugs agb/haftungsausschluss/widerruf/datenschutz), löst die Produkt-Kategorie (`admin_config.products[id].category`) und damit den kategorie-korrekten Höchstbetrag (`getEigenbeteiligung`), und schreibt **freeze-once** (`.is('terms_version', null)`) an die Buchung. **Behebt zugleich B-Bug:** die 10 Aufrufer übergaben nie `productCategory` → der Höchstbetrag fiel immer auf 200 € zurück (360°/Vlog = 300 € wurde falsch angezeigt). Ist schon eingefroren → gefrorene Werte werden verwendet → Verlegung (§ 12 Abs. 2, kein neuer Vertrag) + Verlängerung (§ 13) behalten Fassung + Höchstbetrag. PDF (`contract-template.tsx`): neue Buchungsdaten-Zeile „Einbezogene Fassungen" (via `formatLegalVersions`) + im Hash (`buildContractText`); Basis-Box zeigt `data.eigenbeteiligung` (jetzt korrekt). Download-Route `rental-contract/[bookingId]` bevorzugt den gefrorenen `liability_max_amount` + zeigt die Versionen. Pure Helfer `lib/contracts/legal-snapshot-utils.ts` (`computeLiabilityMaxAmount`/`formatLegalVersions`, JSX-frei → testbar; Test `lib/contracts/__tests__/legal-snapshot-utils.test.ts`). **Backfill:** bestehende unterschriebene Verträge → `terms_version='unbekannt (Altbestand)'` (NICHT mit aktuellem Stand gefüllt = Falschangabe vermeiden; `terms_snapshot_at` bleibt NULL, freeze-once greift nicht mehr). **Go-Live TODO:** Migration ausführen (ohne sie läuft alles defensiv weiter — Snapshot wird nicht persistiert, PDF rendert trotzdem korrekt).
- **Vertrags-Download liefert das signierte Original + Integritäts-Hash (Stand 2026-07-27):** Mietvertrag § 22 Abs. 2 (gespeicherter Vertragstext wird ausgeliefert) + Beweiswert der Signatur. Das Original wird bei Vertragsschluss längst persistiert (`storeContract` → Bucket `contracts`, Pfad `contracts/{jahr}/{bookingId}.pdf`, `rental_agreements` mit `pdf_url`/`contract_hash`/`signed_at`/`ip_address`); `/api/rental-contract/[bookingId]` liefert es aus (konto + Admin verlinken beide dorthin, Admin-`send-email` hängt NUR das Storage-Original an, die Bestätigungsmail hängt den erzeugten-und-gespeicherten Buffer an — kein Pfad rendert das Original neu). **Neu:** (1) Migration `supabase/supabase-rental-agreements-pdf-hash.sql` (additiv: `rental_agreements.pdf_sha256`) + `storeContract` speichert den **SHA-256 der PDF-Datei-Bytes** (`lib/contracts/pdf-hash.ts`, getrennt vom logischen `contract_hash`); die Download-Route **verifiziert** beim Ausliefern die Bytes gegen den Hash und **protokolliert** eine Abweichung (Manipulation), liefert aber weiterhin das Original — es wird NIE stillschweigend eine Reproduktion untergeschoben. (2) Der **Re-Render-Fallback** (Original nicht ladbar / Altvertrag ohne gespeichertes PDF) wird jetzt bei unterschriebenen Verträgen als **„Reproduktion / Nachdruck — nicht die signierte Originalfassung"** im PDF gekennzeichnet (`RentalContractData.reproductionNote` → amber Banner) + geloggt. Defensiver Insert-Retry ohne `pdf_sha256`, falls Migration aussteht. Test `lib/contracts/__tests__/pdf-hash.test.ts`. **Go-Live TODO:** Migration ausführen (ohne sie: kein Byte-Hash gespeichert → keine Verifikation, sonst unverändert). ⚠️ **Gemeldet (nicht gebaut):** (a) Der Re-Render-Fallback nutzt weiterhin `wiederbeschaffungswert = booking.deposit` (Kaution-Anker-Feld) — nur in der gekennzeichneten Reproduktion, das gespeicherte Original hat die echten Positions-WBW; Rebuild wäre „eigenmächtiges Umbauen" des WBW/Kaution-Modells. (b) Postpone-archivierte Original-Fassungen (`bookings.contract_versions`, `contracts/verlegt/{id}/…`) haben **keinen Abruf-Pfad** (§ 12 Abs. 7 „im Kundenkonto abrufbar") — Archiv existiert, aber keine Download-UI. Beides Entscheidung offen.
- **Ausweis-Sofortlöschung bei Ablehnung + DSGVO-Löschprotokoll (Stand 2026-07-27):** AGB § 2 Abs. 3 / Datenschutz Ziffer 6 („bei Ablehnung unverzüglich"). Der live genutzte Reject-Handler `app/api/admin/verify-customer/route.ts` löschte bei `rejected` bisher nur den Status → die Ausweisdateien blieben bis zum nächsten `dsgvo-cleanup`-Lauf liegen. Jetzt: sofortige Löschung (Storage `id-documents` + DB-Verweise `id_front_url`/`id_back_url` genullt) direkt im Handler + Audit `customer.id_deleted`. Der `dsgvo-cleanup`-Cron (90 Tage ab `verified_at` — bereits korrekt an den Verifizierungsabschluss gekoppelt, nicht ans Ende der Geschäftsbeziehung) protokolliert jetzt **jeden** Löschvorgang per `logAudit` (Zeitpunkt, User-ID, Dateianzahl, Grund — niemals Dateiinhalt).
- **Konto-Lebenszyklus-Fristen an Zusagen angepasst (Stand 2026-07-27):** AGB § 19 Abs. 3 / Datenschutz Ziffer 6/15. `lib/account-lifecycle-config.ts` Defaults korrigiert: inaktiv **730 Tage (24 Monate)** statt 365, Vorwarnungen **≥ 30 Tage** in beiden Fällen (`unverified_grace_hours` 48→720, `inactive_grace_days` 14→30, `unverified_warn_after_days` 28→30); die Vorwarn-Minimums werden im Clamp **erzwungen** (kürzere Admin-Werte werden angehoben). Inaktiv-Pfad (`account-cleanup` Cron) nimmt jetzt Konten mit **jeder** Buchung aus (`userIdsWithAnyBooking` statt nur offener Buchungen — Legal „ohne Buchung"; Buchungshalter fallen unter die 10-Jahres-Aufbewahrung § 19 Abs. 4). Inaktiv-Warnmail: hartkodiertes „über ein Jahr" neutralisiert. Pure `normalizeAccountLifecycleConfig` extrahiert + getestet (`lib/__tests__/account-lifecycle-config.test.ts`). ⚠️ **Bewusst NICHT gebaut (gemeldet, Entscheidung offen):** (1) inaktive No-Booking-Konten werden weiterhin nur **deaktiviert, nicht gelöscht** („danach löschen" fehlt); (2) kein automatisches Anonymisieren von Buchungshaltern nach Ablauf der 10-Jahres-Frist. Beides ist irreversible Löschmechanik → braucht ausdrückliche Freigabe.
- **Admin-Label „Eigenbeteiligung" → „Höchstbetrag der Ersatzpflicht" (Stand 2026-07-27):** `components/admin/HaftungContent.tsx` Anzeige-Labels umbenannt („Fallback-Höchstbetrag der Ersatzpflicht", „Höchstbetrag der Ersatzpflicht pro Kategorie", Premium-Badge „Höchstbetrag 0 €"). DB-Feldnamen `standardEigenbeteiligung`/`eigenbeteiligungByCategory` **unverändert** — dadurch matcht der Terminologie-Guard die Datei weiterhin (Substring in `standardEigenbeteiligung`), der `EIGENBETEILIGUNG_ALLOWLIST`-Eintrag bleibt (Begründung aktualisiert: nur noch DB-Identifier). Vollständige Entfernung aus der Allowlist wäre nur mit Umbenennung des DB-Felds möglich (Teil D untersagt das) — gemeldet.
...
- **Zahlungsstatus direkt setzen — „Als bezahlt markieren" (Stand 2026-06-19):** In der Preisaufstellung (Reiter „Übersicht") zeigt eine Zahlungs-Zeile entweder grün **„✓ Bezahlt"** (+ Quelle „Stripe" wenn echter Zahlungseingang) oder amber **„Zahlung ausstehend"** mit grünem Button **„✓ Als bezahlt markieren"**. Spart den Umweg über `/admin/buchhaltung`. Endpoint **`POST /api/admin/booking/[id]/mark-paid`** (Permission via Prefix `/api/admin/booking` → `tagesgeschaeft`): `{ paid?: boolean, method?, date?, note? }`. Beim Markieren wird — falls noch keine `invoices`-Row existiert — eine idempotent angelegt (`storeInvoiceForBooking`, Steuer-Config aus `admin_settings`) und dann auf `status/payment_status='paid'` gesetzt (offene Mahnungen → `paid`). Eine `awaiting_payment`-Buchung wird zusätzlich auf `confirmed` gehoben (sonst würde der Auto-Storno-Cron sie doch noch stornieren). `{ paid: false }` setzt die Rechnung wieder auf `open` (Korrektur) — eine **echte Stripe-Zahlung** kann so NICHT zurückgesetzt werden (`stripe_transactions` bleibt matched → UI-Hinweis). Quelle der Wahrheit bleibt die `invoices`-Row, exakt wie der Bezahlt-Haken im Dashboard. **GET `/api/admin/booking/[id]`** liefert dafür neu `booking.paid` (bool) + `booking.paid_via` (`'stripe'|'invoice'|'derived'`) — gleiche 3-stufige Logik (Stripe → invoices → payment_intent_id-/Status-Fallback). Audit `booking.mark_paid` / `booking.mark_unpaid`. Reine Buchhaltungs-Markierung — kein Stripe-Charge, keine E-Mail.
...
- **Kameras-Tab: EIN gemeinsamer Kalender für alle Kameras (Stand 2026-06-14):** Statt pro Kameramodell ein eigener aufklappbarer Bereich + eigene Tabelle ist der Kameras-Tab jetzt **eine einzige durchgehende Gantt-Tabelle** mit gemeinsamem Datums-Header + gemeinsamem horizontalem Scroll. Jede Zeile = ein physisches Exemplar (Seriennummer); Modelle werden über einen `<tbody>`-Gruppenkopf pro Produkt (Modellname + Kamera-Anzahl) optisch getrennt, bleiben aber im selben Kalender. Kameras ohne Seriennummern erscheinen als kompakter Hinweis-Block unter dem Kalender (Link auf den Kamera-Editor), nicht mehr als leere aufklappbare Karte. Der Aufklapp-/Zuklapp-State (`expandedProducts`/`toggleProduct`) ist entfernt; neue Memos `cameraProducts` (mit Units) + `emptyCameraProducts` (ohne). Reine Layout-Umstrukturierung in `app/admin/verfuegbarkeit/page.tsx` — `cameraAssignment`/`getCellInfo`/`getBookingSpan` + Backend unverändert. Damit erscheint eine Einzel-Kamera-Buchung weiterhin nur auf genau **einer** Zeile (siehe Greedy-Fix unten), jetzt aber im gemeinsamen Kalender direkt sichtbar.
...
- **Kompatible-Kameras-Pills im Zubehör-Header (Stand 2026-05-25):**
  ...
  Client-Fetch).
- **Filter-Leiste pro Tab (Stand 2026-06-14):** Jeder Tab (Kameras/Sets/Zubehör) hat über dem Kalender eine Filter-Leiste mit (a) **Freitext-Suche** (Kameras: Modellname ODER Seriennummer/Label der Einheit; Sets/Zubehör: Name) und (b) **Multi-Select-Chips** (mehrere gleichzeitig wählbar, leer = alle): Kameras nach **Modell** (`product.id`), Zubehör nach **Kategorie** (`accessories.category`), Sets nach **Badge** (`sets.badge`). Der bestehende Kamera-Kompatibilitäts-Dropdown bleibt zusätzlich im Sets-/Zubehör-Tab. State: `search` (geteilt, pro aktivem Tab angewendet) + drei separate Sets `camModelFilter`/`accCategoryFilter`/`setBadgeFilter`. Gefiltert über die Memos `visibleCameraGroups` (Modell-Multiselect + Freitext, gibt pro Modell die passenden Einheiten zurück — `getCellInfo`/`cameraAssignment` rechnen weiter über ALLE Einheiten, nur die Anzeige wird gefiltert), `visibleAccessories`, `visibleSets`. Tab-Counter zeigen die gefilterte Anzahl, leerer Stand zeigt „Keine Treffer für die aktiven Filter", „✕ Filter zurücksetzen" leert Suche + Chips + Kamera-Dropdown. Reine Frontend-Anzeige, kein API-Change.
...
Sektion mit (a) **Bestellinhalt** und (b) **Preisaufstellung** (Miete · Zubehör ·
...
- **Set → Kamera zugeordnet (Stand 2026-08-10):** Bei Multi-Kamera-Buchungen
  (z. B. GoPro + OSMO, je ein „Basic Set") erschienen im ersten Wurf zwei
  gleichnamige „Basic Set"-Blöcke ohne Kamera-Bezug — nicht unterscheidbar. Jetzt
  ist jedes Set eine **eigene Karte** mit einem **Kamera-Badge** („📷 GoPro
  Hero13 Black"). Zuordnung im Endpoint: `sets.product_ids` → Buchungs-Kamera per
  `product_id`-Match (`resolveBookingCameras`), Fallback über den Katalog-Namen
  (`getProducts`, für Legacy-Kameras ohne `product_id`). Kein Badge, wenn das Set
  keiner Buchungs-Kamera eindeutig zuzuordnen ist. Set-Teile stehen als
  aufgelöste Liste in der Set-Karte, direkt gewähltes Zubehör in einer eigenen
  Karte „Zusätzliches Zubehör".
- **Endpoint** `GET /api/booking/[id]/details` (Cookie-Auth, kunden-eigen via
  ...
  Ausgelieferten entspricht), baut die flache Liste in **Gruppen** um (Set-
  Container = `!isFromSet && !accessory_id`, danach die `isFromSet`-Sub-Items;
  gleichnamige Sets bleiben durch die Reihenfolge/Set-ID getrennt — NICHT nach
  `setName` gruppieren) und liefert die Preis-Komponenten (`price_rental`/
  `price_accessories`/`price_haftung`/`shipping_price` + Summe aller Rabattfelder
  + `price_total`/`deposit`). `select('*')` → defensiv gegen fehlende
  Migrationen (`early_bird_discount`/`special_discount`/`sale_items`/
  `booking_type`/`cameras`/`product_ids` optional). Rein lesend, gibt nur
  abgeleitete Felder zurück.
- **UI** (`app/konto/buchungen/page.tsx`): `expandedIds`-Set + `toggleDetails`,
  ...
  Fetch pro Buchung, Spinner/Fehler-Zustand), rendert Set-/Zubehör-Karten +
  Preis-Karte im dunklen Theme. Keine Migration — die Preis-/Zubehör-Spalten
  existieren längst.
...
- **Manuelle Exemplar-Auswahl als Scan-Fallback (Stand 2026-06-09):** Im Packer-Schritt (`PackStep`) hat jede Zubehör-Position einen `📋 Wählen`-Button (über die neue optionale `onManualPick`-Prop der `ItemList` in `components/admin/scan-workflow.tsx` — ohne die Prop unverändert, also keine Regression in Kontrolle/Retoure/Übergabe/Live-Liste). Klick öffnet `<ManualExemplarPicker>` (Modal, dunkles Theme): bei individuell getracktem Zubehör eine Checkbox-Liste der physischen Exemplare (Exemplar-Code + Status, reservierte vorausgewählt, Auswahl auf benötigte Menge gedeckelt); die angehakten Unit-IDs landen in `scannedAccessoryUnitIds` und werden beim Submit **wie ein Scan** über `applyScannedUnits` erfasst (exemplar-genaues Schadenstracking). Bei Sammel-/untracked-Zubehör (`is_bulk` oder keine Einzel-Exemplare) fällt der Picker auf einen reinen Mengen-Stepper zurück (anonym, checkt N Slots, kein Unit-Recording — wie der Bulk-Scan). Datenquelle: neuer Lese-Endpoint `GET /api/admin/booking/[id]/accessory-exemplars?accessory_id=X` (erbt `tagesgeschaeft`-Permission, **nicht** der `katalog`-permissionierte `/api/admin/accessory-units`). Liest **bewusst die Legacy-Tabelle `accessory_units`** — die einzige, die `applyScannedUnits` versteht; der Inventar-Mirror hält sie auch für Neue-Welt-Stücke (individual tracking) gefüllt. Validiert `accessory_id` gegen die set-expandierte Zubehörliste der Buchung (Security, gleiche Expansion wie `scan-lookup`). Reine Frontend-/Lese-Erweiterung — keine Migration, keine Änderung an `applyScannedUnits`.
  - **Auch im Übergabe-Wizard (Stand 2026-06-20):** `ManualExemplarPicker` + `ExemplarStatusPill` + Typ `ExemplarUnit` sind aus `app/admin/versand/[id]/packen/page.tsx` nach `components/admin/scan-workflow.tsx` **verschoben + exportiert** (Pack-Seite importiert sie jetzt von dort, Verhalten 1:1). `app/admin/buchungen/[id]/uebergabe/page.tsx` (Schritt 1 „Zustand bei Übergabe") reicht jetzt ebenfalls `onManualPick` an die `ItemList` durch + rendert den Picker (neue Step1-Props `pickerGroup`/`setPickerGroup`/`scannedAccessoryUnitIds`/`applyManualUnits`/`applyManualQuantity`, Helper im Parent analog Pack-Seite). Damit kann der Admin beim Übergeben pro Zubehör-Position das passende Exemplar aus der Liste der **verfügbaren, zur Buchung gehörenden** Codes wählen (nur die zur gebuchten Kamera passenden, weil pro `accessory_id` aufgelöst — z.B. nur GoPro-Akkus) statt nur blind abzuhaken; gescannte Exemplare sind im Picker vorausgewählt, sodass Scan + Handauswahl gegen die Etiketten abgleichbar sind. Die Übergabe-API (`POST /api/admin/handover/[bookingId]`) erfasst die gewählten Unit-IDs bereits über das bestehende `scannedUnits.accessoryUnitIds`-Feld (`applyScannedUnits`).
  - **Neue-Welt-Fallback im Picker-Endpoint (Stand 2026-06-20):** `GET /api/admin/booking/[id]/accessory-exemplars` las nur die Legacy-Tabelle `accessory_units` → bei Zubehör, das nur in `inventar_units` (neue Welt) lebt und dessen Mirror leer ist, kam `units: []` → der Picker fiel auf den Mengen-Stepper „Keine einzeln erfassten Exemplare hinterlegt" zurück (man konnte den Code nicht wählen). Jetzt: ist die Legacy-Liste leer (und nicht `is_bulk`), enumeriert der Endpoint die individuellen Stücke direkt aus `inventar_units` (über `lookupProdukteId('accessories', …)` → `produkt_id`, `tracking_mode='individual'`, `typ in ('zubehoer','verbrauch')`) und mappt jedes über `migration_audit` (`alte_tabelle='accessory_units'`) auf die Legacy-`accessory_units.id` zurück — fehlt das Mapping, dient die Inventar-ID als Unit-ID, **identisch** zu dem, was ein Scan desselben Codes via `scan-lookup` in `accessory_unit_ids` schreibt. So zeigt der Picker dieselben Codes wie die Scan-Etiketten.
  - **Auch für die Kamera (Stand 2026-06-20):** Der `📋 Wählen`-Button erscheint jetzt auch auf der/den Kamera-Position(en) (`ItemList`-Bedingung um `g.type === 'camera'` erweitert). `ManualExemplarPicker` hat eine neue optionale Prop `fetchUrl` (Default = accessory-Endpoint); für Kameras übergeben Pack- + Übergabe-Seite die URL des neuen Endpoints **`GET /api/admin/booking/[id]/camera-exemplars?product_name=X`**. Dieser liefert die wählbaren Seriennummern des Modells primär aus `product_units` (Name→`product_id` via `getProducts()`, Fallback Buchungs-`product_id`; wählbar = `available` ODER für diese Buchung reserviert via `unit_id`/`cameras[].unit_id`), mit Neue-Welt-Fallback auf `inventar_units` (`typ='kamera'`, `migration_audit`-Rückmapping auf `product_units.id`) — gleiche Bauart wie accessory-exemplars. `product_name` wird gegen die Modelle der Buchung validiert (Komma-Split/`cameras[]`, sonst 403). Beide Seiten branchen den Picker per `pickerGroup.type`: Kamera-Auswahl schreibt über die neue Helper-Funktion `applyManualUnitsCamera` in `scannedCameraUnitIds` (→ `scannedUnits.cameraUnitIds`/`applyScannedUnits`), Zubehör unverändert in `scannedAccessoryUnitIds`. Erbt die `tagesgeschaeft`-Permission über den `/api/admin/booking`-Prefix.
...
#### Verlaengerungs-Zahlungen automatisch zuordnen (Stand 2026-07-26)
Eine Mietverlaengerung (`extend-booking` → `confirm-extension`) hat einen **eigenen** PaymentIntent (Beschreibung „Verlaengerung · Buchung C2R-…", `metadata.type='extension'`, `metadata.booking_id`), der in `bookings.extension_payment_intent_id` landet — **nicht** in `bookings.payment_intent_id`. Deshalb griff keine Stufe der Auto-Match-Kaskade und die Zahlung blieb im Cockpit als „Stripe-Zahlung ohne Buchung" haengen (z. B. die 2,00-€-Verlaengerung von C2R-2627-004). Neue **Stufe 1b** in `lib/buchhaltung/stripe-sync.ts` (direkt nach dem PI-Exact-Lookup):
- **a)** ueber `pi.metadata.type==='extension'` + `pi.metadata.booking_id` → Match auf `bookings.id`.
- **b)** Fallback: Buchung, deren `bookings.extension_payment_intent_id` exakt dieser PI ist (autoritativer Link nach erfolgreichem `confirm-extension`).
- **Kein `hasOtherLink`-/Doppelzahlungs-Check:** die Original-Zahlung derselben Buchung ist schon verknuepft, eine Verlaengerung ist aber eine **legitime Zusatzzahlung**, keine Doppelzahlung. Der Bypass ist eng auf echte Verlaengerungen begrenzt (nur `type='extension'` bzw. gespeicherter Extension-PI) — echte Doppelzahlungen laufen weiter in die Doppelzahlungs-Erkennung.
- **Folge:** `stripe_transactions` bekommt eine zweite `matched`-Zeile fuer dieselbe Buchung (Original + Verlaengerung), `booking_id` hat bewusst keine Unique-Constraint. EÜR/DATEV ziehen aus `bookings.price_total` (enthaelt den Verlaengerungsbetrag bereits) → keine Doppelzaehlung. Die Stripe-Gebuehr der Verlaengerung wird per „Gebuehren als Ausgaben" als eigener Eintrag verbucht (korrekt, es ist eine echte Extra-Charge).
- **Bestand heilt beim naechsten Sync:** unmatched Verlaengerungs-Zahlungen im Zeitraum werden beim naechsten „Synchronisieren"/Cron-Lauf automatisch nachgematcht (nicht `manual`/`refunded` gesetzt → laeuft durch die Kaskade). Keine Migration.
...
#### PayPal-Gebühren-Split (Stand 2026-07-26)
Zahlt der Kunde über Stripe per **PayPal**, zieht Stripe eine **Gesamtgebühr** ab, die zwei Anteile enthält: den Stripe-Anteil (`stripe_fee`) und die **PayPal-Durchleitungsgebühr** (`payment_method_passthrough_fee`). Beide stehen einzeln in `charge.balance_transaction.fee_details[]`. Der „Gebühren als Ausgaben"-Button legt jetzt für solche Zahlungen **ZWEI Ausgaben** an — eine mit Anbieter „Stripe", eine mit Anbieter „PayPal". Reine Kartenzahlungen (keine Durchleitungsgebühr) erzeugen weiterhin genau EINEN Stripe-Eintrag (unverändert).
- **Split-Quelle:** `import-fees` lädt pro Transaktion die Charge mit `expand: ['balance_transaction']`, summiert `fee_details` nach Typ (`payment_method_passthrough_fee` = PayPal, Rest = Stripe). Split greift nur, wenn ein Durchleitungsanteil > 0 existiert UND `stripe + paypal == tx.fee` (±0,02 € Sanity). Sonst Fallback = ein Stripe-Eintrag über die volle Gebühr.
- **Rückerstattungen** werden pro Gebühren-Typ zurückgerechnet (Refund-`fee_details` nach Typ; ohne Detail-Aufschlüsselung → alles dem Stripe-Anteil gutgeschrieben, da PayPal-Durchleitungsgebühren i.d.R. nicht erstattet werden). `effektiv = max(0, brutto + credit)` pro Anteil.
- **Idempotenz:** Stripe-Anteil weiter `source_type='stripe_fee'`, `source_id=tx.id`; PayPal-Anteil `source_id='<tx.id>:paypal'`. Beide Beschreibungen selbstheilend (Prefix `Stripe-Gebühr` bzw. `PayPal-Gebühr`). Das Stripe-Anteil-Vorhandensein bleibt das primäre Gate — ist es schon da, werden nur beide Beschreibungen geheilt, KEINE neuen Stripe-Calls (verhindert nachträgliche Doppelverbuchung). **Kategorie beider Einträge bleibt `stripe_fees`** (Zahlungsabwicklungs-Bucket für EÜR/DATEV) — die Unterscheidung läuft über `vendor` (Stripe/PayPal) + Beschreibung, keine Migration.
- API-Antwort + Audit liefern zusätzlich `paypalImported`; UI-Toast zeigt z. B. „5 Stripe-Gebühren verbucht, 2 PayPal-Gebühren".
...
- **Publish:** Alle 10 Min. Voll-Modus: automatisch. Semi-Modus: nur wenn "Gesehen"-Haken gesetzt.
  ...
  - **Touch-Drag im Kalender (Handy/Tablet, Stand 2026-07-29):** Das Beitrag-Verschieben im Redaktionsplan-Kalender lief nur über natives HTML5-DnD (`draggable`/`onDragStart`/`onDrop`) — das feuert auf Touch-Geräten NICHT, am Handy ließ sich also nichts verschieben. Jetzt zusätzlich Touch-Support in `renderCalendar` (`app/admin/blog/zeitplan/page.tsx`): **Beitrag-Chip kurz gedrückt halten (260 ms Long-Press) → aktiviert Drag-Modus** (leichtes `navigator.vibrate(25)`, Chip wird optisch „angehoben": kräftigeres Bg + Outline, andere Chips dimmen). Bewegung > 12 px VOR dem Long-Press bricht ihn ab (= normales Scrollen). Nach Aktivierung folgt ein `useEffect` mit **non-passiven** `document`-Listenern (`touchmove` mit `preventDefault` gegen Seiten-Scroll, `touchend`/`touchcancel` zum Ablegen); der Ziel-Tag wird per `document.elementFromPoint(...).closest('[data-day]')` (neues `data-day={ds}` an jeder `<td>`) ermittelt und via bestehendem `updateDate(id, day)` gespeichert. `justTouchDragged`-Ref verhindert, dass nach dem Drop das Edit-Modal aufgeht (Tap-Guard in `onClick`). Veröffentlichte Einträge (`isPublished`) bleiben unverschiebbar (Touch-Handler no-op). Desktop-DnD unverändert. Kurzer Hinweistext über dem Kalender. Reine Frontend-Ergänzung — keine API-/Schema-Änderung.
  - **Kalender rückwirkend einsehbar (Stand 2026-07-04):** Der Redaktionsplan-Kalender (`/admin/blog/zeitplan`, `renderCalendar`) zeigte fest den aktuellen Monat + 6 Monate voraus (`for m = 0..6`) — vergangene Einträge (z.B. per „Veröffentlichte nachtragen" eingetragene Live-Beiträge) waren nie sichtbar, obwohl `GET /api/admin/blog/schedule` alle Einträge liefert (nach `scheduled_date` sortiert, kein Datumsfilter). Neuer State `pastMonths` (Default 0), Loop läuft jetzt `for m = -pastMonths..6`. Über dem Kalender: Button **„← 3 frühere Monate anzeigen"** (`pastMonths += 3`), **„Alle bisherigen anzeigen"** (springt zu `earliestBack` = Monatsabstand des frühesten Eintrags), **„Zurücksetzen"** (→ 0) + „N Monate rückwirkend"-Anzeige. Reine Frontend-Anzeige — keine API-/Schema-Änderung. Vergangene Einträge behalten das bestehende Verhalten (veröffentlichte nicht verschiebbar via `isPublishedEntry`).
...
- **Vertragsparagraphen-Editor** (`/admin/legal/vertragsparagraphen` bzw. `/admin/einstellungen?tab=vertrag`): Paragraphen aufklappbar + editierbar, farbcodiert nach Rechtsquelle (AGB/Haftung/Widerruf/Datenschutz), gespeichert in `admin_settings.contract_paragraphs`
  - **Hinzufügen / Löschen / Umsortieren / Kategorie (Stand 2026-07-26):** „+ Paragraph hinzufügen" hängt einen neuen § **am Ende** an (öffnet direkt im Edit-Modus), pro Paragraph im Edit-Panel ein **Kategorie-`<select>`** (AGB/Haftung/Widerruf/Datenschutz), **▲/▼ Umsortieren** und **Löschen** (mit Confirm). Datenmodell erweitert: `{ title, text, category? }` — die Kategorie liegt jetzt **pro Paragraph** im Objekt (Helper `resolveCategory` = `p.category ?? PARAGRAPH_SOURCES[index] ?? 'AGB'`, backward-compatible: Default-19 ohne `category` nutzen weiter die index-basierte `PARAGRAPH_SOURCES`-Map → Badges wandern beim Hinzufügen/Umsortieren nicht mehr). Header-Zähler dynamisch. **Bewusst kein Auto-Renumber** — die §-Nummer steht im `title` und wird vom Admin vergeben (im PDF-Template stehen feste Verweise wie „§ 7"/„§ 9"/„§ 2"). `POST /api/admin/legal/contract-paragraphs` nimmt `category` an + sanitisiert jede Position (title trim/Cap 200, text Cap 8000, category-Allowlist, Einträge ohne title+text verworfen, ≤ 60 Positionen). PDF (`lib/contracts/contract-template.tsx`) rendert unverändert `title`+`text` — `category` ist reines Admin-UI-Feld (kein PDF-Einfluss). GET/DELETE unverändert.
...
- **Katalog:** `lib/email-previews.ts` — `EMAIL_TEMPLATE_CATALOG` listet alle Templates mit id, Name, Trigger-Beschreibung („wann wird die Mail verschickt"), Empfänger (Kunde/Admin) und Render-Funktion
- **Vollständigkeits-Audit (Stand 2026-07-13):** Der Katalog deckt jetzt **alle 45 automatisch versendeten E-Mail-Typen** ab (jeder `emailType`, der irgendwo im Code über `sendAndLog`/`resend` rausgeht) — einzige bewusste Ausnahme ist `test` (Diagnose-Mail aus `/api/admin/test-email`, keine feste Vorlage) und `inbound_received` (eingehende Kundenmail, kein Versand). Ergänzt wurden u.a. `contract_resign_request`, `contract_sign_reminder`, `contract_auto_cancel`, `credit_note`, `kauf_rechnung`, `verification_reminder_manual`, `inbound_reply`, `return_reminder_2d`, `overdue_1d`, `overdue_3d`, `appointment_reminder`, `weekly_report`. Für Mails ohne eigene `sendAndLog`-Builder-Funktion (Rückgabe-/Überfälligkeits-Mails aus `lib/reminder-emails.ts` senden direkt über Resend; Verkaufsrechnung + Auto-Storno + Wochenbericht bauen HTML inline) liegen 1:1-Spiegel-Renderer (`previewReturnReminder2d`, `previewOverdue1d/3d`, `previewSaleInvoice`, `previewContractAutoCancel`, `previewWeeklyReport`) im Katalog — bei Änderung der Original-HTML dort nachziehen. **Neue E-Mail-Typen künftig immer hier ergänzen**, damit die Übersicht vollständig bleibt.
...
- **E-Mail-Rechnungs-Import Go-Live:**
  ...
  3. Unter Buchhaltung → Einstellungen die Adresse eintragen + „E-Mail-Import
     ...
     E-Mail importieren".
- **Vertrags-PDF-Integritäts-Hash Migration auszuführen:** `supabase/supabase-rental-agreements-pdf-hash.sql`
  (idempotent, additiv: `rental_agreements.pdf_sha256`). Ohne sie läuft alles
  weiter (Original wird ausgeliefert), nur der Byte-Hash wird nicht gespeichert
  → keine Integritäts-Verifikation beim Ausliefern. Empfohlen ASAP ausführen.
- **⚠️ Postpone-Fassungen abrufbar machen (§ 12 Abs. 7, Entscheidung offen):**
  Bei einer Verlegung wird die ursprünglich unterzeichnete Fassung nach
  `bookings.contract_versions` (`contracts/verlegt/{id}/…`) archiviert (nicht
  überschrieben) — aber es gibt **keinen Download-Pfad** dafür im Kundenkonto/
  Admin. AGB § 12 Abs. 7 sagt zu, dass die Originalfassung abrufbar bleibt. Eine
  Versions-Download-UI (analog `InvoiceVersionsPanel`) wurde bewusst NICHT
  autonom gebaut (eigene Feature-Fläche) — braucht Freigabe.
- **⚠️ Fallback-Reproduktion nutzt `deposit` als Wiederbeschaffungswert
  (gemeldet):** Der Re-Render-Fallback in `/api/rental-contract/[bookingId]`
  setzt `wiederbeschaffungswert = booking.deposit` (Kaution-Anker aus der
  Produktkonfig). Nur in der gekennzeichneten Reproduktion relevant (das
  gespeicherte Original hat die echten, bei Vertragsschluss eingefrorenen
  Positions-WBW). Der `deposit`-Wert ist ein Rest des Kaution-Modells (AGB
  § 3 Abs. 6 / § 10 Abs. 1: keine Kaution) — Umbau des WBW/Kaution-Modells
  wurde bewusst nicht eigenmächtig vorgenommen.
- **Rechtstext-Versions-Snapshot Migration auszuführen:** `supabase/supabase-bookings-terms-version.sql`
  ...
  + Backfill des Altbestands auf „unbekannt (Altbestand)"). Ohne sie läuft die
  Vertragsgenerierung defensiv weiter (Snapshot wird nicht persistiert, PDF rendert
  trotzdem korrekt); erst NACH der Migration werden die einbezogenen Fassungen + der
  Höchstbetrag pro Buchung eingefroren + im PDF ausgewiesen. Empfohlen ASAP ausführen.
- **⚠️ Konto-Löschung nach Deaktivierung / 10-Jahres-Anonymisierung (Entscheidung offen):**
  Der `account-cleanup`-Cron **deaktiviert** inaktive No-Booking-Konten (24 Monate +
  30 Tage Vorwarnung), **löscht** sie danach aber NICHT (AGB § 19 Abs. 3 „danach
  löschen" fehlt). Zusätzlich fehlt das automatische Anonymisieren von Buchungshaltern
  nach Ablauf der 10-Jahres-Aufbewahrung (§ 19 Abs. 4). Beides ist irreversible
  Löschmechanik und wurde bewusst NICHT autonom gebaut — braucht eine ausdrückliche
