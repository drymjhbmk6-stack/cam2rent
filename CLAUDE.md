...
- **Seite:** Client-Component mit zwei umschaltbaren Ansichten (Präferenz in `localStorage.admin_auftragskalender_view`):
  - **Monat** — 6-Wochen-Raster (Montag-basiert), Statusfarbe (awaiting_payment lila / confirmed cyan / shipped amber / picked_up orange / returned grau / completed grün). Pro Buchung werden bis zu **drei farbige Balken** im gemeinsamen Lane-Packing platziert: der Mietzeitraum-Balken (`rental_from..rental_to`, Statusfarbe) sowie zwei 1-Tages-Aktionsbalken — `ship`-Balken am `ship_date` und `return`-Balken am `return_date`. Die Aktionsbalken sind **schraffiert** (`repeating-linear-gradient` via `stripedBg()`) und nach **Lieferart** gefärbt, nicht nach raus/zurück: Versand (Hin- + Rückversand) = amber `ACTION_COLORS.versand`, Abholung (Übergabe + Rückgabe) = indigo `ACTION_COLORS.abholung`. Label je Event „📤 Versand"/„📤 Übergabe" bzw. „📥 Rückversand"/„📥 Rückgabe". Lane-Packing über alle drei Event-Typen via `WeekEvent` (`kind: 'rental'|'ship'|'return'`), Aktionsbalken haben einen dezenten weißen Rand zur Abgrenzung. Pro Tag nur noch das `📝 N`-Notiz-Badge (die alten 📤/📥-Tages-Badges sind durch die Balken ersetzt). Balken-Klick → `/admin/buchungen/[id]`, Tag-Klick → Notiz-Modal.
