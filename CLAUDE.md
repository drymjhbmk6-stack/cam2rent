...
- **Bestandteil-Bilder-Migration auszuführen:** `supabase/supabase-accessories-included-parts-images.sql` (idempotent). Fügt nullable `accessories.included_parts_images TEXT[] DEFAULT '{}'`. Ohne Migration läuft alles weiter (defensiver Retry-Pfad in POST/PUT `accessories`: Spalte droppen + Warnung), Bild-Upload-Endpoint funktioniert, aber die Bild-URLs werden nicht persistiert. Bucket `product-images` wird wiederverwendet (Pfad `accessories/<id>/parts/...`).
