...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#0a0f1e' }}>
      {/* Sidebar */}
      <aside className="w-56 shrink-0 flex flex-col" style={{ background: '#0f172a', borderRight: '1px solid #1e293b' }}>

        {/* Logo */}
        <Link href="/admin" className="block px-5 py-5" style={{ borderBottom: '1px solid #1e293b', textDecoration: 'none' }}>
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          <NavSection label="Shop" items={SHOP_ITEMS} pathname={pathname} />
          <div style={{ height: 1, background: '#1e293b', margin: '6px 12px' }} />
          <NavSection label="Bestellungen & Kunden" items={ORDERS_ITEMS} pathname={pathname} />
          <div style={{ height: 1, background: '#1e293b', margin: '6px 12px' }} />
          <NavSection label="Inhalte" items={CONTENT_ITEMS} pathname={pathname} />
          <div style={{ height: 1, background: '#1e293b', margin: '6px 12px' }} />
          <NavSection label="Daten" items={DATA_ITEMS} pathname={pathname} />
        </nav>
      ...
      {/* Main Content */}
      <main className="admin-dark flex-1 min-w-0 overflow-auto" style={{ background: '#0a0f1e', color: '#e2e8f0' }}>
        {children}
      </main>
    </div>
  );
