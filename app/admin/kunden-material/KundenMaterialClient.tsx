...
export default function KundenMaterialClient({
  initialEntries,
  initialCounts,
}: {
  initialEntries: UgcEntry[];
  initialCounts: Counts;
}) {
  const searchParams = useSearchParams();
  const initialOpen = searchParams.get('open');

  // Startwerte server-gerendert (SSR, Default-Filter 'pending') → sofort im ersten Paint.
  const [entries, setEntries] = useState<UgcEntry[]>(initialEntries);
  ...
  useEffect(() => {
    ...
    // Erst-Render: die SSR-Daten (Default-Filter 'pending') sind schon da.
    if (first && initialEntries.length > 0) {
      setCached(ugcCacheKey('pending'), { entries: initialEntries, counts: initialCounts });
      return;
    }
    void load();
