...
function RankTable({ title, items, unit }: { title: string; items: RankItem[]; unit: string }) {
  ...
  return (
    <div style={{ background: 'var(--admin-surface)', borderRadius: 12, padding: 16, border: '1px solid var(--admin-border)' }}>
      <h2 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 12px' }}>{title}</h2>
      {items.length === 0 ? (
        <p style={{ color: 'var(--admin-text-dim)', fontSize: 13, margin: 0 }}>Keine Daten im Zeitraum.</p>
      ) : (
        <div style={{ display: 'grid', gap: 8 }}>
          {items.map((it, idx) => (
            <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 22, color: 'var(--admin-text-dim)', fontSize: 12, textAlign: 'right', flexShrink: 0 }}>{idx + 1}.</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--admin-accent)', fontWeight: 700, flexShrink: 0 }}>{it.count}× {unit}</span>
                </div>
                <div style={{ height: 6, background: 'var(--admin-surface-2)', borderRadius: 999, marginTop: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${(it.count / max) * 100}%`, background: 'var(--admin-accent)', borderRadius: 999 }} />
                </div>
              </div>
            </div>
          ))}
...
function BarRow({ label, count, max }: { label: string; count: number; max: number }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <span style={{ width: 140, fontSize: 13, color: 'var(--admin-text-2)', flexShrink: 0 }}>{label}</span>
      <div style={{ flex: 1, height: 18, background: 'var(--admin-surface-2)', borderRadius: 6, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${max > 0 ? (count / max) * 100 : 0}%`, background: '#0891b2', borderRadius: 6 }} />
      </div>
      <span style={{ width: 44, textAlign: 'right', fontSize: 13, fontWeight: 700, color: 'var(--admin-text)', flexShrink: 0 }}>{count}</span>
    </div>
  );
}
    ...
    <div style={{ padding: '20px 16px', color: 'var(--admin-text)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <PageHeader
          ...
          subtitle="Anonyme Auswertung: welche Kamera, welches Zubehör und welcher Mietzeitraum im Buchungsprozess konfiguriert wurden (erfasst beim Erreichen der Zusammenfassung — ohne Kundendaten)."
          actions={
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        color: active ? 'var(--admin-primary-text)' : 'var(--admin-text-2)',
                        fontSize: 12,
                        fontWeight: 700,
                        cursor: 'pointer',
                      }}
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    border: '1px solid var(--admin-border)',
                    background: mode === 'custom' ? 'var(--admin-accent)' : 'var(--admin-surface)',
                    color: mode === 'custom' ? 'var(--admin-primary-text)' : 'var(--admin-text-2)',
                    fontSize: 12,
                    fontWeight: 700,
                    cursor: 'pointer',
                  }}
              ...
              {mode === 'custom' && (
                <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  ...
                  />
                  <span style={{ fontSize: 13, color: 'var(--admin-text-dim)' }}>bis</span>
                  <input
                    type="date"
                    value={customTo}
                    min={customFrom}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border: 'none',
                      background: customValid ? '#10b981' : 'var(--admin-surface-2)',
                      color: customValid ? '#0a0a0a' : 'var(--admin-text-dim)',
                      fontSize: 12,
                      fontWeight: 700,
        ...
        {loading ? (
          ...
          <p style={{ color: 'var(--admin-danger)' }}>Fehler beim Laden.</p>
        ) : data.migration_pending ? (
          <div style={{ background: 'var(--admin-surface)', borderRadius: 12, padding: 32, textAlign: 'center', border: '1px solid rgba(239,68,68,0.35)' }}>
            <p style={{ color: 'var(--admin-danger)', fontSize: 16, fontWeight: 700, margin: 0 }}>Migration ausstehend</p>
            <p style={{ color: 'var(--admin-muted)', fontSize: 13, marginTop: 8, marginBottom: 0 }}>
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            <div style={{ background: 'var(--admin-surface)', borderRadius: 12, padding: 16, border: '1px solid var(--admin-border)' }}>
              <p style={{ fontSize: 12, color: 'var(--admin-muted)', textTransform: 'uppercase', letterSpacing: 0.5, margin: 0 }}>
                Konfigurationen{headline ? ` — ${headline}` : ''}
              </p>
              <p style={{ fontSize: 32, fontWeight: 800, color: 'var(--admin-accent)', margin: '4px 0 0' }}>{data.total}</p>
            </div>
