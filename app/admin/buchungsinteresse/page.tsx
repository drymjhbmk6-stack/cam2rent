...
export default function BuchungsinteressePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/booking-interest?days=${days}`);
            ...
            {RANGES.map((r) => (
              <button
                ...
                style={{
                  ...
                  background: days === r.days ? '#06b6d4' : '#111827',
                  color: days === r.days ? '#0a0a0a' : '#cbd5e1',
                  fontSize: 12,
                  fontWeight: 700,
                  cursor: 'pointer',
                }}
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            <div style={{ background: '#111827', borderRadius: 12, padding: 16, border: '1px solid #1e293b' }}>
              <p style={{ fontSize: 12, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5, margin: 0 }}>
                Konfigurationen — letzte {data.days} Tage
              </p>
