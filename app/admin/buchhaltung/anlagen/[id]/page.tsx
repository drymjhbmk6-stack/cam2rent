...
export default function AnlageDetailPage() {
  ...
  async function changeStatus(newStatus: string) {
    if (!(await confirm({ message: `Status auf "${newStatus}" setzen? Dies erzeugt eine Sonder-AfA.`, danger: true }))) return;
    setBusy(true);
    const res = await fetch(`/api/admin/anlagen-neu/${id}`, {
  ...
  if (!asset) return <div className="p-6 text-admin-muted">{error ?? 'Lädt…'}</div>;
    ...
    <div className="min-h-screen text-admin-text px-4 sm:px-6 py-6">
      ...
      <div className="max-w-4xl mx-auto mt-4 space-y-6">
        <div>
          ...
          <p className="text-sm text-admin-muted">{asset.art} · angeschafft {fmtDate(asset.anschaffungsdatum)}</p>
        </div>
        ...
        {inventarUnit && (
          <section className="bg-admin-surface border border-admin-border rounded p-4">
            ...
            <p className="text-xs text-admin-muted mt-1">→ Wiederbeschaffungswert findest du dort.</p>
          </section>
        )}
        ...
        <section className="bg-admin-surface border border-admin-border rounded p-4">
          ...
          {history.length === 0 ? (
            <p className="text-sm text-[var(--admin-text-dim)] italic">Noch keine AfA gebucht.</p>
          ) : (
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return <div className="flex justify-between text-sm"><span className="text-admin-muted">{label}</span><span className="font-mono">{value}</span></div>;
}
