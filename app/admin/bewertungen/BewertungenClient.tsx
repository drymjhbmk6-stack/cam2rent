...
export default function BewertungenClient({ initialReviews }: { initialReviews: Review[] }) {
  // Startwert server-gerendert (SSR, Default-Filter 'all') → sofort im ersten Paint.
  ...
  useEffect(() => {
    ...
    // Erst-Render: die SSR-Daten (Default-Filter) sind schon da → nicht neu laden.
    if (first && initialReviews.length > 0) {
      setCached(reviewsCacheKey(filter), initialReviews);
      return;
    }
    // Filterwechsel (oder leeres SSR): Cache für diesen Filter → sofort anzeigen,
    // still revalidieren. Sonst Spinner.
