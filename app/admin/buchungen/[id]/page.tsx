...
function BookingAccessoryEditSection({
  ...
  const [editing, setEditing] = useState(false);
  const [rows, setRows] = useState<{ id: string; qty: number }[]>(initialRows);
  const [reason, setReason] = useState('');
  const [priceOn, setPriceOn] = useState(false);
  const [priceVal, setPriceVal] = useState<string>(String(priceTotal ?? 0));
  ...
  function start() {
    ...
    setReason('');
    setPriceOn(false);
    setPriceVal(String(priceTotal ?? 0));
    setErr('');
    setEditing(true);
  }

  async function save() {
    const items = rows.filter((r) => r.id).map((r) => ({ accessory_id: r.id, qty: r.qty }));
    if (reason.trim().length < 10) {
      setErr('Bitte einen Grund mit mindestens 10 Zeichen angeben.');
      return;
    }
    ...
    setErr('');
    try {
      const payload: { items: typeof items; reason: string; new_price_total?: number } = {
        items,
        reason: reason.trim(),
      };
      if (priceOn) {
        ...
        if (!Number.isFinite(n) || n < 0) {
          setErr('Bitte einen gültigen Preis (≥ 0) angeben.');
          setSaving(false);
          return;
        }
        payload.new_price_total = n;
      }
      const res = await fetch(`/api/admin/booking/${bookingId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accessory_edit: payload }),
      });
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
  ...
  if (!editing) {
    return (
      <Section title="Zubehör der Buchung bearbeiten">
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs text-brand-muted leading-relaxed">
              Ändert die echte Buchung — wirkt in Packliste, Übergabeprotokoll,
              Scan-Workflow und Verfügbarkeit. Mietvertrag bleibt unverändert.
            </p>
            <button
              type="button"
              onClick={start}
          ...
          <div className="bg-brand-bg-soft rounded-lg p-3 space-y-1">
            {leafRows.length === 0 ? (
              <p className="text-xs text-brand-muted italic">Kein Zubehör gebucht.</p>
            ) : (
              leafRows.map((r, i) => (
                <div key={i} className="flex justify-between items-baseline gap-3 text-sm">
                  <span className="font-body text-brand-black truncate">
                    {r.name}
                    {r.isFromSet && r.setName && (
                      <span className="text-xs text-brand-muted"> (aus Set: {r.setName})</span>
                    )}
                  </span>
                  <span className="text-xs font-heading font-semibold text-brand-muted">{r.qty}×</span>
                </div>
              ))
            )}
          </div>
  ...
  return (
    <Section title="Zubehör der Buchung bearbeiten">
      <div className="space-y-4">
        <p className="text-xs text-brand-muted leading-relaxed">
          Echte Änderung der Buchung. Verfügbarkeit wird hart geprüft —
          ist ein hinzugefügtes/erhöhtes Teil im Zeitraum nicht frei, wird die
          Änderung abgelehnt. Set-Teile erscheinen einzeln; nach dem Speichern
          sind sie eigenständige Positionen.
        </p>
        ...
        <div className="space-y-2">
          {rows.map((row, i) => (
            <div key={i} className="flex items-center gap-2">
              <select
                value={row.id}
                onChange={(e) => setRows((rs) => rs.map((r, j) => (j === i ? { ...r, id: e.target.value } : r)))}
                className="flex-1 min-w-0 text-base border border-brand-border rounded-lg px-2 py-2"
              >
                <option value="">— wählen —</option>
                {setOptions.length > 0 && (
              ...
              <input
                ...
                max={99}
                value={row.qty}
                onChange={(e) => setRows((rs) => rs.map((r, j) => (j === i ? { ...r, qty: Math.max(1, Math.min(99, Number(e.target.value) || 1)) } : r)))}
                className="w-16 text-base border border-brand-border rounded-lg px-2 py-2"
              />
              <button
                type="button"
          ...
          <button
            type="button"
            onClick={() => setRows((rs) => [...rs, { id: '', qty: 1 }])}
            className="text-sm text-accent-blue font-heading font-semibold"
          >
            + Zubehör hinzufügen
          </button>
          {rows.filter((r) => r.id).length === 0 && (
            <p className="text-xs text-brand-muted">Keine Zeile = Buchung ohne Zubehör.</p>
          )}
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={2}
            className="w-full text-base border border-brand-border rounded-lg px-3 py-2"
            placeholder="z.B. Kunde hat Adapter nachträglich dazugebucht"
          />
        </div>
        ...
        <div className="rounded-lg border border-brand-border p-3 space-y-2">
          ...
          {priceOn && (
            <>
              ...
              />
              <p className="text-xs text-brand-muted leading-relaxed">
                Neuer Brutto-Gesamtbetrag. <strong>Keine automatische
                Stripe-Buchung</strong> — Nach- oder Rückzahlung separat klären.
                Eine bereits erzeugte persistente Rechnung wird nicht
