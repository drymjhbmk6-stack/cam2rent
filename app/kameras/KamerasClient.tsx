...
export default function KamerasClient({ initialProducts }: { initialProducts?: Product[] }) {
  const ctx = useProducts();
  // Server-Daten fuer den ersten Frame; sobald der Provider live geladen hat,
  // hat er Vorrang (Live-Bestand/-Verfuegbarkeit). Memoisiert, damit die
  // Referenz stabil bleibt (nachgelagerte useMemo/Filter rechnen nicht bei
  // jedem Render neu).
  const products = useMemo(
    () => (ctx.products.length > 0 ? ctx.products : (initialProducts ?? [])),
    [ctx.products, initialProducts],
  );
