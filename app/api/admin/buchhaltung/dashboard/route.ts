...
export async function GET(req: NextRequest) {
  ...
  // Vorzeitraum berechnen (gleiche Länge)
  ...
  // Rechnungen laden (für Offene Posten)
  ...
  const openAmount = (openInvoices || []).reduce((sum, inv) => sum + (inv.gross_amount || 0), 0);

  // Bezahlte Rechnungen im Zeitraum
  ...
  // Kundenname aus Rechnungen anreichern
  const openDunning = (dunningData || []).map(d => ({
    id: d.id,
    invoice_number: '',
    customer_name: '',
    days_overdue: 0,
  ...
  // Rechnungen für Mini-Tabelle anreichern
