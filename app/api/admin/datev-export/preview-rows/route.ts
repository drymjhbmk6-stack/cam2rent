...
export async function GET(req: NextRequest) {
  ...
  const previewCols = 'id, product_name, customer_name, price_rental, price_accessories, price_haftung, shipping_price, discount_amount, duration_discount, loyalty_discount, early_bird_discount, special_discount, refund_amount, status, created_at';
  // Optionale Spalten, die je nach ausstehender Migration fehlen koennen.
  const OPTIONAL_PREVIEW_COLS = [', early_bird_discount', ', special_discount', ', refund_amount'];
  ...
  if (previewErr && /refund_amount|early_bird_discount|special_discount|column|schema cache|PGRST/i.test(previewErr.message)) {
  ...
  for (const b of ((bookings ?? []) as unknown as PreviewRow[])) {
    ...
    // Alle Rabatt-Arten abziehen (nicht nur discount_amount) — konsistent zum
    // CSV-Export: duration/loyalty/early_bird/special.
    const rental = (b.price_rental || 0) + (b.price_accessories || 0)
      - (b.discount_amount || 0)
      - (b.duration_discount || 0)
      - (b.loyalty_discount || 0)
      - (b.early_bird_discount || 0)
      - (b.special_discount || 0)
      - (b.refund_amount || 0);
    if (rental > 0) {
      rows.push({
        datum,
