...
export default function FavoritenPage() {
  ...
  if (!user) {
    return (
      <div className="bg-white rounded-card shadow-card p-8 text-center">
        <p className="text-brand-steel text-sm">Bitte melde dich an, um deine Favoriten zu sehen.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="font-heading font-bold text-xl text-brand-black">Meine Favoriten</h1>
      ...
      {favProducts.length === 0 ? (
        ...
        <div className="bg-white rounded-card shadow-card p-10 text-center">
          ...
          <p className="font-heading font-semibold text-brand-black mb-1">Noch keine Favoriten</p>
          <p className="text-sm text-brand-steel mb-5">
            Klicke auf das Herz bei einer Kamera, um sie hier zu speichern.
          </p>
          <Link
            href="/kameras"
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-black text-white font-heading font-semibold text-sm rounded-btn hover:bg-brand-dark transition-colors"
          >
            Kameras entdecken
          </Link>
        </div>
              ...
              >
                ...
                <div className={`${colors.bg} px-5 py-4 flex items-center justify-between`}>
                  <div>
                    ...
                    <h3 className="font-heading font-semibold text-base text-brand-black mt-0.5">
                      {product.name}
                    </h3>
                  </div>
                  <button
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <p className="text-sm text-brand-steel line-clamp-2 mb-3">
                    {product.shortDescription}
                  </p>

                  <div className="mt-auto flex items-center justify-between">
                    <div className="flex items-baseline gap-1">
                      <span className="text-xs text-brand-steel">ab</span>
                      <span className="font-heading font-bold text-lg text-brand-black">
                        {product.pricePerDay.toFixed(2).replace('.', ',')} €
                      </span>
                      <span className="text-xs text-brand-steel">/ Tag</span>
                    </div>
                    <Link
                      href={`/kameras/${product.slug}`}
                      className="inline-flex items-center gap-1.5 px-4 py-2 bg-brand-black text-white font-heading font-semibold text-xs rounded-btn hover:bg-brand-dark transition-colors"
                    >
                      Ansehen
